(function () {
  ...
  const defaultCoupleNames = `${data.brideName || ""} & ${data.groomName || ""}`.trim();
  const defaultCoupleNamesText = `${data.brideName || ""} dhe ${data.groomName || ""}`.trim();
  const coupleNames = data.eventTitle || defaultCoupleNames;
  const coupleNamesText = data.eventTitle || defaultCoupleNamesText;
  const coupleInitials = (data.eventInitials || `${(data.brideName || "").trim().charAt(0)} & ${(data.groomName || "").trim().charAt(0)}`).toUpperCase();
  const rsvpEventText = data.rsvpEventText || `dasmën e ${coupleNamesText}`;
  ...
  function setTextFields() {
    const values = {
      coupleNames,
      coupleInitials,
      introCardLabel: data.eventTitle ? "" : "DASMA E",
      introCardText: data.eventTitle || coupleInitials,
      ...
      els.openingVideo.addEventListener("loadeddata", () => {
        if (els.openingVideo.paused && els.openingVideo.readyState >= 2) {
          try {
            ...
            // The first video frame appears as soon as the browser allows seeking.
          }
        }
      }, { once: true });
  ...
  function openWhatsApp(isAttending) {
    const message = isAttending
      ? `Përshëndetje, konfirmoj pjesëmarrjen time në ${rsvpEventText}.` 
      : `Përshëndetje, më vjen keq, nuk mund të marr pjesë në ${rsvpEventText}.`;
  ...
  function setupIntro() {
    ...
    const finishIntro = () => {
      ...
      els.openingVideo.pause();
      els.introScreen.classList.add("show-white-initials");
      ...
      setTimeout(() => els.introScreen.remove(), 2150);
    };
